...
const NewsFeedService = () => {
     ...
     * @returns 
     */
    const createFeed = async (feedData: newsFeed, email: string) => {
        try {
     ...
     * @param email 
     */
    const fetchFeedByEmail = async (email: string) => {
        try {
     ...
     * @returns 
     */
    const fetchFeedByID = async (feedId: number) => {
        try {
     ...
     * @param email 
     */
    const findFeed = async (feedId: number, email: string) => {
        try {
    ...
    const deleteFeed = async (feedId: number) => {
        try {
            await Feed.deleteOne({id: feedId}).lean();
        } catch(err: any) {
            const error: any = new Error(err?.message);
            error.statusCode = APP_CONSTANTS.STATUS_CODES.INTERNAL_SERVER_ERROR;
            throw error;
 ...
 */
const likeFeed = async (feedId: number, email: string) => {
    try {
     ...
     * @returns 
     */
    const feedComment = async (feedId: number, comments: object) => {
        try {
    ...
    return {createFeed, fetchFeed, fetchFeedByID, fetchFeedByEmail, deleteFeed, findFeed, likeFeed, feedComment};
}
